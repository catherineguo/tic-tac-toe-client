...
var createGameFail = function createGameFail() {
...
var shareCurrentPlayer = function shareCurrentPlayer() {
  return currentPlayer;
};

var canAddPlayer = function canAddPlayer(indexNum) {
...
var updateCurrentBoard = function updateCurrentBoard(indexNum) {
  currentBoard[indexNum] = currentPlayer;
  checkWin();
};

var updateSuccess = function updateSuccess(indexNum, response) {
  store.game = response.game;
...
var isBoardFull = function isBoardFull(spot) {
  if (spot) {
    return true;
  } else {
    return false;
...
module.exports = {
  ...
  shareCurrentPlayer: shareCurrentPlayer,
...
var addCurrentPlayer = function addCurrentPlayer(indexNum, currentPlayer, winOrDraw) {
...
var addMark = function addMark(indexNum, currentPlayer) {
  var emoji = '';
  if (currentPlayer === 'x') {
    emoji = '🍟';
  } else {
...
var turnMessage = function turnMessage(currentPlayer) {
  if (currentPlayer === 'o') {
...
var winMessage = function winMessage(winner) {
  if (winner === 'x') {
...
var invalidMoveMessage = function invalidMoveMessage() {
...
var onGetGamesSuccess = function onGetGamesSuccess(response) {
  ...
  var checkArrayWinner = function checkArrayWinner(array) {
    var topRow = [array[0], array[1], array[2]];
    var middleRow = [array[3], array[4], array[5]];
    var bottomRow = [array[6], array[7], array[8]];
    var leftColumn = [array[0], array[3], array[6]];
    var middleColumn = [array[1], array[4], array[7]];
    var rightColumn = [array[2], array[5], array[8]];
    var leftDiagonal = [array[0], array[4], array[8]];
    var rightDiagonal = [array[2], array[4], array[6]];
    var isX = function isX(spot) {
      if (spot === 'x') {
        return true;
      } else {
        return false;
    ...
    if (topRow.every(isX) || middleRow.every(isX) || bottomRow.every(isX) || leftColumn.every(isX) || middleColumn.every(isX) || rightColumn.every(isX) || leftDiagonal.every(isX) || rightDiagonal.every(isX)) {
      return true;
    } else {
      return false;
    }
  ...
  var calculateWins = function calculateWins() {
    var wins = 0;
  ...
  var totalWins = calculateWins();

  if (numOfGames === 0) {} else {
    numOfGames--;
  }
  if (incompleteGames === 0) {} else {
    incompleteGames--;
  }
...
var onGetGamesFail = function onGetGamesFail(response) {
...
var gameEngine = __webpack_require__(2);
var gameApi = __webpack_require__(4);
var ui = __webpack_require__(3);
...
var newGame = function newGame() {
  gameApi.createGame().then(gameEngine.createGameSuccess).catch(gameEngine.createGameFail);
};

var updateSquare = function updateSquare(indexNum) {
  if (!gameEngine.canContinue()) {} else if (!gameEngine.canAddPlayer(indexNum)) {
    ui.invalidMoveMessage();
  } else {
    var value = gameEngine.shareCurrentPlayer();
    gameEngine.updateCurrentBoard(indexNum);
    var boolean = !gameEngine.canContinue();
    gameApi.updateGame(indexNum, value, boolean).then(function (response) {
      gameEngine.updateSuccess(indexNum, response);
    }).catch(gameEngine.updateGameFail);
  }
};

var toggleGameStats = function toggleGameStats(event) {
  gameApi.getGames().then(ui.onGetGamesSuccess).catch(ui.onGetGamesFail);
};

var addHandlers = function addHandlers() {
  var _loop = function _loop(i) {
    $('#square' + i.toString()).on('click', function () {
      updateSquare(i);
    });
  };
...
var showSignUp = function showSignUp(event) {
  $('#sign-up-container').show();
  $('#sign-in-container').hide();
  $('#user-auth-message').html('').removeClass('alert alert-danger');
};

var showSignIn = function showSignIn(event) {
...
var onSignIn = function onSignIn(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  authApi.signIn(data).then(authUi.signInSuccess).then(gameApi.createGame).then(gameEngine.createGameSuccess).catch(authUi.signInFail);
};
...
var toggleChangePassword = function toggleChangePassword(event) {
...
var signOutClear = function signOutClear() {
  $('#sign-up input').val('');
  $('#sign-in input').val('');
  $('#change-password input').val('');
